...
interface ContentProgressModalProps {
  isOpen: boolean
  onClose: () => void
  onComplete: () => void
}

export const ContentProgressModal: React.FC<ContentProgressModalProps> = ({
  isOpen,
  onClose,
  onComplete,
}) => {
  ...
  return (
    <ModalShell
      isOpen={isOpen}
      onClose={onClose}
      title="Agent sedang membuat konten"
      titleId="content-progress-title"
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        >
          ...
          >
            {progressStep >= 4 ? '✓' : progressStep === 3 ? '●' : '○'} Menyiapkan resource dan tugas
          </div>
          <div
            style={{
          ...
          >
            {isDone
              ? '✓ Kelengkapan konten terverifikasi'
              : progressStep === 3
              ? '● Memvalidasi kelengkapan konten'
              : '○ Memvalidasi kelengkapan konten'}
          </div>
        </div>
        ...
        >
          ...
          >
            {isDone ? 'Review Konten' : 'Menyiapkan Konten...'}
          </button>
        </div>
      </div>
    </ModalShell>
